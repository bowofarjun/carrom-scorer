...
export default function Index() {
  ...
  const launch = () => { if (!toss) return; Haptics.selectionAsync(); setMatch((m) => ({ ...m, breakIndex: toss === "A" ? 0 : 2 })); setUndoStack([]); setMode("active"); };
  ...
  const undoLastBoard = () => {
    const doUndo = () => {
      ...
      if (previousState) {
        setMatch(JSON.parse(previousState));
    ...
    if (Platform.OS === "web") {
      if (typeof window !== "undefined" && window.confirm("Undo Last Board?\n\nThis will revert the score, player seats, and the break order to exactly how they were before the last board was completed.")) {
        doUndo();
      }
      return;
    }
  ...
  const completeBoard = () => {
    ...
    setUndoStack((prev) => [...prev, JSON.stringify(match)]);
    ...
    setMatch((m) => {
      ...
      // FIX: If boards are unlimited (0), crossover ONLY happens at 13 points.
      const hitHalfwayBoard = m.boardsPerSet > 0 && m.board === Math.floor(m.boardsPerSet / 2);
      const needsCrossover = !setDone && isFinalSet && !m.finalCrossoverDone && 
          (hitHalfwayBoard || scoreCrossed13);
  ...
  const reset = () => { const doReset = () => { AsyncStorage.removeItem("carrom-match"); setMatch(defaultMatch); setMode("setup"); setA(""); setB(""); setA2(""); setB2(""); setToss(null); setWinner(null); setMen(1); setQueen(false); setQueenEligible(true); setUndoStack([]); }; if (Platform.OS === "web") { if (typeof window !== "undefined" && window.confirm("Start a new match?\n\nThe current board history will be cleared.")) doReset(); return; } Alert.alert("Start a new match?", "The current board history will be cleared.", [{ text: "Cancel", style: "cancel" }, { text: "New match", style: "destructive", onPress: doReset }]); };
